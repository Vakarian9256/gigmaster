...
class Database:
    ...
    def create_new_artist_list(self, user_id: int) -> str:
        ...
        self.user_collection.update_one(
            {"_id": user_id}, {"$set": {"artists_id": artists_id}}
        )
        return artists_id
    ...
    def create_new_shown_concerts_list(self, user_id: int) -> str:
        ...
        self.user_collection.update_one(
            {"_id": user_id}, {"$set": {"shown_concerts_id": concerts_id}}
        )
        return concerts_id
    ...
    def add_artist(self, user_id: int, artist: str):
        ...
        if artist not in artists:
            ...
            self.artists_collection.update_one(
                {"_id": artists_id}, {"$set": {"artists": artists}}
            )
    ...
    def remove_artist(self, user_id: int, artist: str):
        ...
        if artist in artists:
            ...
            logger.warning(
                "Removing %s from user id %s list of artists", artist, user_id
            )
            self.artists_collection.update_one(
                {"_id": artists_id}, {"$set": {"artists": artists}}
            )
    ...
    def add_concerts(self, user_id: int, concerts: List[Dict]):
        self.check_if_user_exists(user_id, raise_exception=True)
        concerts_id = self.user_collection.find_one({"_id": user_id})[
            "shown_concerts_id"
        ]
        shown_concerts = self.shown_concerts_collection.find_one({"_id": concerts_id})[
            "shown_concerts"
        ]
        ...
        self.shown_concerts_collection.update_one(
            {"_id": concerts_id}, {"$set": {"shown_concerts": shown_concerts}}
        )
